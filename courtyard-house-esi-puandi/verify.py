#!/usr/bin/env python3
"""Verify CityHovi prototype: images, booking widget, layout at multiple viewports."""
...
def main():
    with sync_playwright() as p:
        ...
        toast_visible = page.locator("#inquiryToast").evaluate(
            "el => el.classList.contains('show')"
        )
        results["interactions"].append(f"inquiry toast shown: {toast_visible}")
        if not toast_visible:
            results["errors"].append("Inquiry toast did not show")
        ...
        page.screenshot(
            path=str(ARTIFACTS / "cityhovi-desktop-full.png"), full_page=True
        )
        ...
        page.screenshot(
            path=str(ARTIFACTS / "cityhovi-mobile-full.png"), full_page=True
        )
