#!/usr/bin/env python3
"""Verify Hotel CityHovi prototype: images, booking widget, layout at multiple viewports."""
...
def main():
    with sync_playwright() as p:
        ...
        # Hero uses CSS background — verify it loads via fetch
        hero_ok = page.evaluate(
            """async () => {
              ...
              if (!m) return false;
              try {
                const r = await fetch(m[1], { method: 'GET', mode: 'cors' });
                return r.ok;
              } catch (e) {
                // cors may block fetch; try image probe
        ...
        results["images"].append({"src": "hero-bg", "ok": bool(hero_ok)})
        if not hero_ok:
            results["errors"].append("Broken hero background image")
        ...
        page.screenshot(path=str(ARTIFACTS / "cityhovi-desktop-full.png"), full_page=True)

        page.set_viewport_size({"width": 390, "height": 844})
        page.wait_for_timeout(500)
        page.locator("#menuToggle").click()
        page.wait_for_timeout(400)
        page.screenshot(path=str(ARTIFACTS / "cityhovi-mobile-menu.png"))
        ...
        page.locator("#guestTrigger").click()
        page.wait_for_timeout(300)
        page.screenshot(path=str(ARTIFACTS / "cityhovi-mobile-booking.png"))

        page.evaluate("window.scrollTo(0, document.body.scrollHeight)")
        page.wait_for_timeout(600)
        page.screenshot(path=str(ARTIFACTS / "cityhovi-mobile-full.png"), full_page=True)
